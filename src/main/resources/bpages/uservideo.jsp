                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <c:forEach items="${ requestScope.videos }" var="video">
                                    <tr>
                                        ...
                                        <td>${video.desc}</td>
                                        <td>
                                            <button name="details" type="button" class="btn btn-primary"
                                                    value="${video.id}">
                                                查看
                                            </button>
                                            <button name="off" type="button" class="btn btn-primary"
                                                    data-toggle="modal" data-target="#delModal" value="${video.id}">下架
                                            </button>
...
<script>
    $(function () {
        ...
        $("[name=off]").click(function () {
            //获取视频名
            var name = $(this).parents("tr").children().eq(1).html();
            $("#delModal").find(".modal-body").html("确认下架视频'" + name + "'?");
            //将管理员id传进模态框
            $("#del").val($(this).val());
            $("#d").val("");
        })
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == "ok") {
                        ...
                        layer.msg("下架成功", {time: 2000}, function () {
                            location.href = "http://localhost:8080/mvideo/VideoServlet?action=managerVideoList";
                        });
                    }
                }
        ...
        $("[name=details]").click(function () {
            var nickName = $(this).parents("tr").children().eq(2).html();
            var category = $(this).parents("tr").children().eq(3).html();
            location.href = "http://localhost:8080/mvideo/bpages/videodetails.jsp?id=" + $(this).val() + "&nickName=" + nickName + "&category=" + category;
        })
